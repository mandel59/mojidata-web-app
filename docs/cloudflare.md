...
When running Playwright smoke tests against a remote green preview, pass the same
release asset base used by the browser bundle so the setup warm-up does not
require legacy app-local `/assets/*` routes:

```sh
PLAYWRIGHT_TEST_SPA_ASSET_BASE_URL='https://mojidata-spa-assets.mandel59.workers.dev/releases/<release-id>' \
npm run test:e2e:target -- \
  --base-url 'https://<preview-alias>-mojidata-web-app.mandel59.workers.dev' \
  -- tests/e2e/spa.spec.ts --project=chromium \
  --grep 'search-spa renders results in browser|mojidata-spa renders character data in browser'
```
