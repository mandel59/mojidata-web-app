...
- Browser SPA asset negotiation: Worker `mojidata-spa-assets`, which redirects
  `sql-wasm.wasm`, `moji.db`, and `idsfind.db` to Brotli-compressed R2 objects
  for browsers that can decode them.
...
one step with `sql-wasm.wasm`, `moji.db`, and `idsfind.db` all routed through
`mojidata-spa-assets`.
...
`/assets/sql-wasm.wasm`, `/assets/moji.db`, and `/assets/idsfind.db` to the
public R2 asset origin. It chooses `.br` or raw R2 objects from
...
The same Worker also routes `idsfind.db` and `sql-wasm.wasm` to their Brotli R2
objects for browsers that advertise `br`; Safari/WebKit still receives raw DB
assets.
